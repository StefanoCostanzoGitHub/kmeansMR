...
type Kmeans struct {
	// Master port: Default to 8000
	port int
	// index of available mappers
	mappers []int
	withCombiner bool
...
func NewWithOptions(numMap int, maxIters int, deltaThreshold float64, withCombiner bool, plotter Plotter) (Kmeans, error) {
...
func (a *API) MapReduce(input utils.Input, reply *utils.Output) error {
	...
	count := 0
	for iter := 0; ; iter++ {
		// 2.0) Init variables and empty channels
		changes := 0
		kvs := make(utils.InRed, 0, nPoints)
		numMapper := len(km.mappers)
		emptyChannels(chInMap, chOutMap)

		// 2.1) send new clusters to mapper
		for i := 0; i < numMapper; i++ {
		...
		for i := 0; i < numMapper; i++ {
			...
			elapsed += time.Since(start) /////////////////////777
		}
		fmt.Println("elapsed: ", elapsed) ///////////////////7
	...
	*reply = utils.Output{Cc: cc, NPoints: nPoints}
	return nil
}
...
func main() {
	...
	log.Printf("\nMapReduce started: Master is listening on port %d", km.port)

	server.Accept(listener)
}

/**
// 2.5) Plotter (DEV) servira?
if km.plotter != nil {
	err := km.plotter.Plot(cc, i) // (DEV) attenzione la i è cambiata
	if err != nil {
		return fmt.Errorf("failed to plot chart: %s", err)
	}
}**/
